    ...
    <aside class="hidden md:flex flex-col bg-[#1A1A1A] w-64 shadow px-2">
      ...
      <nav class="flex-1">
        <ul class="space-y-2">
          <router-link
            to="/tasks"
            class="block w-full px-4 py-2 text-[#F5F5F5] font-semibold transition-colors duration-300 hover:bg-[#333333]"
          >
            My Tasks
          </router-link>
          <router-link
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex items-center space-x-4">
          <div class="relative" ref="notificationMenuRef">
            ...
            <div v-if="isNotificationsOpen" class="absolute right-0 mt-2 w-72 bg-[#F5F5F5] rounded-md shadow-lg py-1 z-50">
              <div v-if="limitedNotifications.length === 0" class="px-4 py-2 text-sm text-gray-500">
                No new notifications.
              </div>
              <div v-else>
                <div 
                  v-for="(notif, index) in limitedNotifications" 
                  :key="index"
          ...
          <div v-if="authStore.user" class="relative" ref="profileMenuRef">
            <button @click="toggleProfileMenu" class="flex items-center space-x-2 rounded-full p-2 bg-[#F5F5F5] hover:bg-[#CCCCCC] focus:outline-none focus:ring">
